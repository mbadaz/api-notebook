...
beforeAll(async () => {
  ...
  const meta = wsfs.createWorkspace('Live WS', wsDir);
  appData.addWorkspaceEntry({ id: meta.id, path: meta.path });
  wsId = meta.id;
  makeRequest = (over) => ({ ...wsfs.defaultRequest('r', 'r', 'websocket'), ...over });

  // In-test WebSocket echo server (the "target").
  echo = new WebSocketServer({ port: 0, host: '127.0.0.1' });
  echo.on('connection', (s) => {
    s.on('message', (m, isBinary) => s.send(m, { binary: isBinary }));
  });
  await new Promise<void>((r) => echo.once('listening', () => r()));
  echoUrl = `ws://127.0.0.1:${portOf(echo)}`;
...
interface Frame {
  t: string;
  sessionId: string;
  payload?: { data?: string; message?: string; code?: number };
}

describe('live channel — WebSocket', () => {
  it('connects, echoes a message, and closes', async () => {
    ...
    const request = makeRequest({
      websocket: { url: echoUrl, subprotocols: '', messages: [] },
    });
    client.send(
    ...
    client.send(JSON.stringify({ t: 'send', sessionId: 'S1', payload: { data: 'hello' } }));
  ...
  it('reports an error for an invalid target URL', async () => {
    ...
    await new Promise<void>((r) => client.once('open', () => r()));
    const request = makeRequest({
      websocket: { url: 'http://not-websocket/', subprotocols: '', messages: [] },
    });
    ...
    expect(f.payload?.message).toMatch(/ws:\/\/|wss:\/\//);
    client.close();
  });
});
